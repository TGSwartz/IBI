...
SorwateClean <- function(estateName, noMerra = F) {
  
# set first date and end of files to load

firstYear <- year <- 2002
firstMonth <- month <- 007
firstDay <- day <- 04
...
# set points data to pull approriate cells near sorwate (or other plantation if desired)
...
for (i in 1:length(merraVars)) {
  ...
  wd <- "/Volumes/Tom Passport/merra/"
  outWd <- "/Users/Tom/Documents/IBI/sorwateArea"
  ...
  while (as.Date(Date) <= as.Date(paste(endYear, endMonth, endDay, sep = "-"))) { 
    ...
    if(file.exists(file)) {
      ...
      rasterName <- crop(rasterName, DegreePointExtent(
        subset(points, plantationName == estateName), .1))  
  ...
  write.csv(merra, paste("/Users/Tom/Documents/IBI/sorwateArea/", estateName, "Merra_", merraVariable, ".csv", sep = ""), row.names = F)
...
write.csv(chirps, paste("/Users/Tom/Documents/IBI/sorwateArea/", estateName, "Chirps.csv", sep = ""), row.names = F)
...
write.csv(arc, paste("/Users/Tom/Documents/IBI/sorwateArea/", estateName, "Arc.csv", sep = ""), row.names = F)

#tamsat <- read.csv(paste("/Users/Tom/Documents/IBI/sorwateArea/", estateName, "11kmTAMSAT.csv", sep = ""))
...
#write.csv(tamsat, paste("/Users/Tom/Documents/IBI/sorwateArea/", estateName, "Tamsat.csv", sep = ""), row.names = F)

wd <- "/Volumes/Tom Passport/evi/"
...
while (as.Date(Date) < as.Date(paste(endYear, endMonth, endDay, sep = "-"))) { 
  ...
  if(file.exists(file)) {
    ...
    rasterName <- crop(rasterName, 
                       MeterPointExtent(subset(points, plantationName == estateName), 10000))
    print(file) # print names to check progress
  }
  else {
    print("no file": paste(wd, "MYD13A1_", year, "-",  month, "-", day, 
                           ".500m_16_days_EVI.tif", sep = ""))
    break
  }
...
write.csv(bimonthEvi, paste("/Users/Tom/Documents/IBI/sorwateArea/", estateName, "BimonthEvi.csv", sep = ""), row.names = F)
